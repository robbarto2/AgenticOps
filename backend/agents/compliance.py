...
async def compliance_node(state: AgentState, writer: StreamWriter) -> dict:
    ...
    for _ in range(max_iterations):
        if time.time() - loop_start > AGENT_LOOP_TIMEOUT_SEC:
            logger.warning("Compliance agent hit %ds loop timeout", AGENT_LOOP_TIMEOUT_SEC)
            break
        try:
            ...
            logger.error("Compliance agent: LLM call timed out after 60s")
            break
        ...
        for tool_call in response.tool_calls:
            ...
            source = "meraki"
            if tool_name.startswith("te_") or "thousandeyes" in tool_name.lower() or tool_name in (
    ...
    if needs_forced_summary(response, max_iterations, "Compliance", logger, len(tool_results)):
        messages.append(HumanMessage(content=FORCE_SUMMARY_PROMPT))
        try:
            response = await asyncio.wait_for(llm.ainvoke(messages), timeout=60)
        except Exception:
            logger.error("Compliance forced summary also failed")
